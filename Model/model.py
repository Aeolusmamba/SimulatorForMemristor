...
class Model():
    def __init__(self, file_path, batch_size: int, hyper_p: dict):
        ...
        in_out = Variable([batch_size, int(df.loc[0]['IFM channel depth']), int(df.loc[0]['IFM height']),
                           int(df.loc[0]['IFM width'])],
        ...
        for i, row in df.iterrows():
            ...
            if row['kernel height'] == 1 and row['kernel width'] == 1:  # which is a linear layer
                ...
                # batch norm
                if not pd.isna(row['BatchNorm']) and row['BatchNorm'] == 1:
                    bn = BatchNorm(input_variable=in_out, name=name + '＿BatchNorm')
                    in_out = bn.output_variable
                    self.parameters.append(bn.gamma)
                    self.parameters.append(bn.beta)
                    self.linear_list.append(bn)
                ...
                if row['bias'] == 1:
                    ...
                    bias = False
                conv = Conv2D(kernel_shape, input_variable=in_out, name=name, hyper_p=hyper_p, stride=stride,
                ...
                # optimizer
                if not pd.isna(row['optimizer']) and row['optimizer'].lower() == 'adam':
                    conv.weight.set_method_adam(beta1=hyper_p['beta1'], beta2=hyper_p['beta2'],
                                                epsilon=hyper_p['epsilon'])
                    if bias:
                        conv.weight_bias.set_method_adam(beta1=hyper_p['beta1'], beta2=hyper_p['beta2'],
                                                         epsilon=hyper_p['epsilon'])
                elif not pd.isna(row['optimizer']) and row['optimizer'].lower() == 'nga':
                ...
                # batch norm
                if not pd.isna(row['BatchNorm']) and row['BatchNorm'] == 1:
                   bn = BatchNorm(input_variable=in_out, name=name + '_BatchNorm')
                   in_out = bn.output_variable
                   self.parameters.append(bn.gamma)
                   self.parameters.append(bn.beta)
                   self.conv_list.append(bn)
    ...
    def forward_propagation(self, X, phase) -> Variable:
        # assign the real input to the first layer's input_variable.data
        self.conv_list[0].input_variable.data = X
        for conv in self.conv_list:
            conv.forward(phase)

        self.linear_list[0].input_variable.data = self.conv_list[-1].output_variable.data.reshape(
            self.linear_list[0].input_variable.shape)

        # self.linear_list[0].input_variable.data = X.reshape(self.batch_size, -1)
    ...
    def back_propagation(self):
        ...
        for linear in reversed(self.linear_list):
            linear.backward()
            # if "relu" not in linear.name:
            #     print(f"weight diff of {linear.name}: ", linear.weight.diff)
        self.conv_list[-1].output_variable.diff = self.linear_list[0].input_variable.diff.reshape(
            self.conv_list[-1].output_variable.shape)
