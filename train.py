...
parser.add_argument('--epochs', type=int, default=50, help='the number of epochs to train (default: 50)')
parser.add_argument('--lr', type=float, default=0.2, help='learning rate (won\'t change overtime)')
parser.add_argument('--grad_scale', type=float, default=1, help='learning rate (will change overtime) for wage delta calculation')
parser.add_argument('--dr', type=float, default=0.01, help='decay rate')
...
parser.add_argument('--loss', default='MSE', help='choose the loss function (MSE or CrossEntropy')
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                if args.loss == 'MSE':
                    ...
                    # check softmax
                    if not isinstance(model.linear_list[-1], Softmax):
                        raise Exception("The last activation function should be Softmax if using CrossEntropy.")
                loss.forward()
            ...
            if epoch % args.test_epoch_interval == 0:
                ...
                for i, (data, target) in enumerate(test_loader):
                    ...
                    assert args.loss in ['MSE', 'CrossEntropy'], args.loss
                    if args.loss == 'MSE':
                        loss = MSE([data, label], name='MSE')
                    else:
                        loss = CrossEntropy([data, label], name='CrossEntropy')
                    loss.forward()
